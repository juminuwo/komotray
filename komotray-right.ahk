...
global MonitorIndex := 2  ; Right monitor (elements[2] in JSON)
global MonitorName := "Right Monitor"
...
; Set up pipe with unique name for right monitor
PipeName := "komotray-right"
...
Loop {
    ...
    ; Update tray icon for right monitor only
...
UpdateMonitorIcon(State) {
    ; Check if right monitor exists
    if (State.Monitors.elements.Length() < MonitorIndex) {
        ; Right monitor doesn't exist, exit gracefully
        Menu, Tray, Tip, Right Monitor: Not Connected
        return
    }

    ; Get right monitor information
    ...
    if (NewState != MonitorIconState) {
        ...
        if (FileExist(icon)) {
            ...
            ; Fallback icon
            Menu, Tray, Icon, % IconPath . "1-2.ico"
        }

        ; Create tooltip showing right monitor information
        TooltipText := "Monitor 2: Workspace " . (Workspace + 1) . " on " . Monitor.name
...
WaitForKomorebi() {
    ...
    Loop, %MaxRetries% {
        ...
        if (ErrorLevel) {
            Menu, Tray, Tip, Right Monitor: Connected to Komorebi
            return true
        }

        if (A_Index = 1) {
            Menu, Tray, Tip, Right Monitor: Waiting for Komorebi to start...
        } else {
            Menu, Tray, Tip, Right Monitor: Waiting for Komorebi... (%A_Index%/%MaxRetries%)
        }
...
SubscribeToKomorebi(PipeName, Pipe) {
    ...
    Loop, %MaxRetries% {
        try {
            ...
            Menu, Tray, Tip, Right Monitor: Connected to Komorebi events
            return true
        } catch e {
            if (A_Index < MaxRetries) {
                Menu, Tray, Tip, Right Monitor: Connecting to Komorebi... (%A_Index%/%MaxRetries%)
                Sleep, %RetryDelay%
            }
        }
    }
