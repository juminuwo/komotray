...
WaitForKomorebi() {
    ...
    Loop, %MaxRetries% {
        ...
        if (ErrorLevel) {
            Menu, Tray, Tip, Left Monitor: Connected to Komorebi
            return true
        }

        if (A_Index = 1) {
            Menu, Tray, Tip, Left Monitor: Waiting for Komorebi to start...
        } else {
            Menu, Tray, Tip, Left Monitor: Waiting for Komorebi... (%A_Index%/%MaxRetries%)
        }
...
SubscribeToKomorebi(PipeName, Pipe) {
    ...
    Loop, %MaxRetries% {
        try {
            ...
            Menu, Tray, Tip, Left Monitor: Connected to Komorebi events
            return true
        } catch e {
            if (A_Index < MaxRetries) {
                Menu, Tray, Tip, Left Monitor: Connecting to Komorebi... (%A_Index%/%MaxRetries%)
                Sleep, %RetryDelay%
            }
        }
    }
